...
const stepDefs = [
  {
    ...
    description: 'Export a CSV or OFX file from your bank and upload it here. Takes about 30 seconds.',
    href: '/upload',
  },
  {
    number: 2,
    ...
    description: 'Drag transactions into categories. We use this to map your spending behavior.',
    href: '/categorize',
  },
  {
    number: 3,
    ...
    description: 'See your Money Personality, spending patterns, and full Financial Autopsy.',
    href: '/dashboard',
  },
]
...
export function OnboardingWelcome({ uploadsDone, uncategorizedCount }: Props) {
  ...
  let ctaLabel: React.ReactNode = (
    <><UploadCloud size={16} />Upload &amp; Reveal My Money Personality &rarr;</>
  )
  if (activeStep === 2) {
    ctaHref = '/categorize'
    ctaLabel = <><Tags size={16} />Categorize {uncategorizedCount} transaction{uncategorizedCount === 1 ? '' : 's'} &rarr;</>
  } else if (activeStep === 3) {
    ctaHref = '/dashboard'
    ctaLabel = <><BarChart2 size={16} />Go to Dashboard &rarr;</>
  }
  ...
  return (
    <div style={{
      ...
      boxShadow: '0 1px 2px rgba(0,0,0,0.08), 0 12px 40px rgba(0,0,0,0.18)',
      display: 'flex',
      ...
      <div style={{ textAlign: 'center', maxWidth: 540 }}>
        <div style={{
          ...
          background: 'linear-gradient(135deg, rgba(124,137,255,0.22) 0%, rgba(139,111,255,0.18) 100%)',
        ...
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.32)', margin: 0, fontStyle: 'italic', letterSpacing: '0.01em' }}>
          ...
          return (
            <div key={step.number} style={{
              background: isActive
                ...
                : isDone ? 'rgba(22,163,74,0.06)' : 'var(--card)',
              border: isActive
                ? '1px solid rgba(124,137,255,0.38)'
                : isDone ? '1px solid rgba(22,163,74,0.30)' : '1px solid var(--border)',
              borderRadius: 14, padding: '14px 16px',
              display: 'flex', flexDirection: 'column', gap: 8,
              opacity: isActive || isDone ? 1 : 0.55,
              boxShadow: isActive
                ? '0 0 0 1px rgba(124,137,255,0.12), 0 4px 20px rgba(124,137,255,0.10)'
                : isDone ? '0 0 0 1px rgba(22,163,74,0.08)' : 'none',
              transition: 'box-shadow 200ms ease',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{
                  width: 30, height: 30, borderRadius: 8, flexShrink: 0,
                  background: isDone ? 'rgba(22,163,74,0.18)' : isActive ? 'rgba(124,137,255,0.22)' : 'var(--surface2)',
                  border: isDone ? '1px solid rgba(22,163,74,0.32)' : isActive ? '1px solid rgba(124,137,255,0.32)' : '1px solid var(--border)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  {isDone
                    ? <Check size={15} style={{ color: 'rgb(22,163,74)' }} />
                    : <Icon size={15} style={{ color: isActive ? 'var(--accent)' : 'var(--muted)' }} />
                  }
                </div>
                <span style={{
                  fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase',
                  color: isDone ? 'rgb(22,163,74)' : isActive ? 'var(--accent)' : 'var(--muted)',
                }}>
                  Step {step.number}
                </span>
              </div>
          ...
          {howItWorks.map((item) => {
            ...
            return (
              <div key={item.title} style={{
                ...
                <div>
                  <p style={{ margin: '0 0 3px', fontSize: 12, fontWeight: 700, color: 'var(--text)', lineHeight: 1.2 }}>
                    {item.title}
                  </p>
