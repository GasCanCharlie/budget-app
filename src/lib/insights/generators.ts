...
export function generateOverBudgetDiagnosis(metrics: ComputedInsightMetrics): InsightCard[] {
  ...
  return [
    makeCard(
      ...
      summary,
      data,
      [viewTransactionsAction(`/transactions?year=${year}&month=${month}`), dismissAction()],
      confidence,
      'TrendingDown',
      year,
      month,
  ...
  for (const tx of top3) {
    ...
    cards.push(
      makeCard(
        ...
        summary,
        data,
        [viewTransactionsAction(`/transactions?search=${encodeURIComponent(tx.merchant)}&year=${year}&month=${month}`), dismissAction()],
        'high',
        'CreditCard',
        year,
        month,
...
export function generateSmallPurchaseLeaks(metrics: ComputedInsightMetrics): InsightCard[] {
  ...
  return [
    makeCard(
      ...
      summary,
      data,
      [viewTransactionsAction(`/transactions?maxAmount=15&year=${year}&month=${month}`), dismissAction()],
      'high',
      'Droplets',
      year,
      month,
